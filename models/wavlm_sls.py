...
from models.sls import SLS
from utils.ig_utils import compute_ig_attributions

class WavLM_SLS(WavLMBaseModel):
    def __init__(self, pretrained_path="microsoft/wavlm-base-plus", freeze_wavlm=True,
        ...
        self.sls = SLS(inputs_dim=sls_input_dim, outputs_dim=1)

        # Precomputed mean features for baseline
        self.ig_baseline = torch.load("models/sls_mean.pt", map_location=torch.device('cpu'))

    def forward(self, x):
        # x: [Batch, Time]
        features = self.extract_features(x) # [Nb_Layer, Batch, Time, Dim]
        
        # SLS expects [Nb_Layer, Batch, Time, Dim]
        logits = self.sls(features) # [Batch, 1]
    ...
    def get_attributions(self, waveform, target_class=0):
        ...
        # SLS expects [Nb_Layer, Batch, Time, Dim]
        attributions = compute_ig_attributions(features, self.sls, None, target_class, expanded_baseline)
