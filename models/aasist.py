...
class Residual_block(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.first:
            ...
            out = x
        out = self.conv1(out if not self.first else x)
        out = self.bn2(out)
        out = self.selu(out)
        out = self.conv2(out)
        if self.downsample:
...
class AASIST(nn.Module):
    def __init__(self, inputs_dim=768, outputs_dim=768):
        ...
        self.LL = nn.Linear(self.inputs_dim, 128)
    ...
    def forward(self, x):
        # x: [Nb_Layer, Batch, Frame_len, Dim] -> we take last layer
        x = x[-1] if x.dim() == 4 else x 
        x = self.LL(x) # [B, T, 128]
        
        x = x.transpose(1, 2) # [B, 128, T]
        x = x.unsqueeze(dim=1) # [B, 1, 128, T]
